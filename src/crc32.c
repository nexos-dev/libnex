/*
    crc32.c - computes CRC32 checksum of buffer
...
const uint32_t poly8Lookup[256] = {
...
uint32_t Crc32Calc (const uint8_t* p, size_t bytelength)
{
    uint32_t crc = 0xFFFFFFFF;
    while (bytelength-- != 0)
        crc = poly8Lookup[((uint8_t) crc ^ *(p++))] ^ (crc >> 8);
    return ~crc;
}
